...
from nexusformat.nexus import NXdata, NXentry, NXfield, nxopen, NXgroup, NXlink, NXroot
import warnings
import xarray as xr
...
def write_nexus_file(filename, data_dict, verbose=False):
    ...
    def _write_field(base, key, val):
        ...
        if isinstance(val, dict):
            ...
            if units is not None:
                root[f"{base}/{key}"] = NXfield(data, name=name, units=units)
            else:
                root[f"{base}/{key}"] = NXfield(data, name=name)
        else:
            print("raw data field")
            if isinstance(val, list):
                val = np.array(val)
            try:
                root[f"{base}/{key}"] = NXfield(val, name=key)
    ...
    def write_spec(base_path, spec):
        ...
        if metadata:
            ...
            root[meta_path] = NXgroup()
            for mk, mv in metadata.items():
                root[f"{meta_path}/{mk}"] = NXfield(mv, name=mk)
        ...
        if 'default' in spec:
            ...
            if isinstance(data_ref, str):
                ...
                tmp_name = 'data_array'
                root[f"{base_path}/{tmp_name}"] = NXfield(data_ref, name=tmp_name)
            ...
            root[f"{base_path}/data"] = nxdata
            root[f"{base_path}"].attrs["default"] = "data"
